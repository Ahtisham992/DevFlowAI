...
export function ChatInterface({ projectId }: { projectId?: string }) {
    ...
    // Fetch single project conversation if applicable
    useEffect(() => {
        if (projectId) {
            api.get(`/ai/projects/${projectId}/conversation`).then((res) => {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const res = await fetch(`${baseUrl}/ai/chat/stream`, {
                ...
                body: JSON.stringify({ message: currentInput, conversationId, model: selectedModel, projectId }),
